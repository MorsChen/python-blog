...
def delete(id):
    post = Posts.query.filter_by(id=id).first()
    if current_user.is_admin or Posts.query.filter_by(id=id, author_id=current_user.id).first():
...
def del_comment(id):
    ...
    if current_user.is_admin or Comments.query.filter_by(id=comment.post_id, author_id =  current_user.id).first() :
